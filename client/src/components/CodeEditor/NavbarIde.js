  ...
  return (
      <div className="navbar-b">
			  <div className='main2-b'>
			    <h1 className={`ic-b ${iconClassName}`}> CodeBlaZe </h1>
			    <h1 className={`icon1-b ${iconClassName}`} onClick={() => setIconClassName(iconClassName === 'green' ? 'red' : 'green')}>
              <SiCompilerexplorer className='icon2-b' />
          </h1>
			  </div>
            
        <div className="icon-options-b">
          {languages.map(lang => (
            <div key={lang.value} className={`icon-option-b ${userLang === lang.value ? 'selected' : ''}`} onClick={() => handleLangChange(lang)}>
              {lang.icon}
            </div>
          ))}
        </div>
